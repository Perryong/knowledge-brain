...
TOTAL_LINES=$(( $(wc -l < "$TARGET_ABS") ))   # $(( )) strips macOS wc -l leading pad
...
LIVE_LOCKS=$(( $(bash "$LOCK_SH" list | wc -l) ))   # $(( )) strips macOS wc -l leading pad
...
GARBLED=$(( $(awk 'length > 100' "$TARGET_ABS" | wc -l) ))   # $(( )) strips macOS wc -l leading pad
