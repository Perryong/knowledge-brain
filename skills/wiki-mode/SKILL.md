...
```bash
cat .vault-meta/mode.json | bash scripts/python-bin.sh -c 'import json,sys; print(json.load(sys.stdin)["mode"])'
```
...
if [ -f .vault-meta/mode.json ]; then
  MODE=$(bash scripts/python-bin.sh -c 'import json; print(json.load(open(".vault-meta/mode.json"))["mode"])')
else
  MODE="generic"
fi
```
